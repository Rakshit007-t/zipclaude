...
const CreateLook: React.FC = () => {
  ...
  // STEP 2 — Product search (search global_products by brand or title)
  const handleProductSearch = (val: string) => {
    ...
    searchTimeout.current = setTimeout(async () => {
      setSearching(true);
      try {
        // Load all global products and filter client-side (simple approach)
        const snap = await getDocs(collection(db, 'global_products'));
        const all = snap.docs.map(d => ({ id: d.id, ...d.data() })) as GlobalProduct[];

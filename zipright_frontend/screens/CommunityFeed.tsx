  ...
  const toggleLike = async (look: Look) => {
    ...
    try {
      if (isLiked) {
        ...
        await addDoc(collection(db, 'users', user.uid, 'lookLikes'), {});
        // Use the look.id as document ID — rewrite with setDoc equivalent via addDoc workaround:
        // Actually use: await addDoc but we need the lookId as docId, so use setDoc:
        // Correction — use updateDoc for likes count and a simple addDoc for the like record:
        await updateDoc(doc(db, 'looks', look.id), { likesCount: increment(1) });
      }
    } catch (err) {
      console.error(err);

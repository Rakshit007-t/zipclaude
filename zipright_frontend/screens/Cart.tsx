...
const Cart: React.FC = () => {
  ...
  const handleCheckout = () => {
    ...
    // For now, we'll just show a toast or navigate to a coming soon page
    navigate('/checkout');
  };
              ...
              {items.map((item) => (
                ...
                >
                  <div 
                    ...
                    onClick={() => window.open(item.affiliateLink || item.url, '_blank')}
                  >
                    <img 
                      src={item.image} 
                      alt={item.title} 
      ...
      {items.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-[#111111]/90 backdrop-blur-xl border-t border-white/5">
          <button 
            ...
            className="w-full py-4 bg-[#22c55e] text-black rounded-xl font-black text-sm uppercase tracking-widest active:scale-95 transition-transform flex items-center justify-center gap-2"
          >

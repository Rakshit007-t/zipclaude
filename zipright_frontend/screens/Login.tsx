...
import { signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, RecaptchaVerifier, signInWithPhoneNumber, GoogleAuthProvider, signInWithCredential } from 'firebase/auth';
...
const Login: React.FC = () => {
  ...
  const createSessionAndNavigate = async (user: any) => {
    const userId = user.uid || user.id;
    // Initialize Member Profile in Firestore if not exists
    try {
      const membersRef = collection(db, 'members');
      const q = query(membersRef, where('uid', '==', userId), where('isPrimary', '==', true));
      const querySnapshot = await getDocs(q);
      
      if (querySnapshot.empty) {
        await addDoc(membersRef, {
          uid: userId,
          name: user.firstName || 'My Fits',
          isPrimary: true,
          fitData: {
            brand: 'Nike',
            topSize: 'M',
      ...
      console.error("Error initializing primary member:", e);
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      createSessionAndNavigate(appUser);
    } catch (err: any) {
      console.error(err);
  ...
  const handleSaveProfile = async () => {
      ...
      try {
        ...
        const appUser = {
            ...
            phone: user.phoneNumber || `${selectedCountry.code}${phone}`,
            provider: 'phone',
            photoUrl: ''
        };
        ...
        <AnimatePresence mode="wait">
          {step === 'input' && (
            ...
            >
              ...
              {/* TODO: REMOVE - Skip for testing */}
              <button 
                onClick={() => navigate('/fit-profile', { state: { mode: 'add' } })}
                className="mt-6 py-3 px-6 rounded-full border border-red-500/30 text-red-400 text-[10px] font-bold uppercase tracking-widest mx-auto block hover:bg-red-500/10 transition-colors"
              >
                Skip Login (Testing)
              </button>

    ...
    <>
      ...
      {isZMenuOpen && (
        <>
          ...
          />
          <div className="fixed inset-x-0 bottom-20 z-[60] px-4" style={{ animation: 'zMenuSlideIn 0.25s ease-out' }}>
      ...
      <nav className="fixed bottom-0 left-0 z-50 w-full bg-[#111111] border-t border-white/5 shadow-2xl">
...
const AppContent: React.FC<{ user: User | null; loading: boolean }> = ({ user, loading }) => {
  ...
  useEffect(() => {
    if (user) {
      setProfileImage(user.photoURL); // Initial fallback
      const userRef = doc(db, 'users', user.uid);
      const unsubscribe = onSnapshot(userRef, (doc) => {
        if (doc.exists()) {
          setZipPoints(doc.data().zipPoints || 0);
      ...
      const inboxRef = collection(db, 'users', user.uid, 'friend_inbox');
      const unsubInbox = onSnapshot(inboxRef, (snap) => {
        setUnreadFriends(snap.docs.filter(d => !d.data().seen).length);
      });
  ...
  return (
      <div className="w-full min-h-screen min-h-dvh bg-[#111111] relative overflow-hidden transition-colors duration-300">
        <Routes>
